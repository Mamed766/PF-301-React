...
const Header = () => {
    ...
    const { addDark, addLight } = useContext(LigtOrDarContext)
        ...
        <header className="w-full text-gray-700 bg-white border-t border-gray-100 shadow-sm body-font">
            <div className="container flex flex-col flex-wrap items-center p-5 mx-auto md:flex-row">
                ...
                <nav className="flex flex-wrap items-center text-base lg:w-2/5 md:ml-auto">
                    {
                        navbarItems && navbarItems.map((nav, index) => (
                            <Link key={nav.id} to={nav.url} className={addActiveClass(nav.url)}>{nav.name}</Link>
                        ))
                    }
                </nav>
                <Link
                ...
                <div className="inline-flex items-center h-full ml-5 lg:w-2/5 lg:justify-end lg:ml-0">
                    <Link className="mr-5 font-medium hover:text-gray-900">Login</Link>
                    <Link
                    ...
                    <div className='flex items-center gap-3 ml-4'>
                        <IoSunnyOutline
                            onClick={addLight}
                            size={20} className="text-2xl text-yellow-500 cursor-pointer" />
                        <FaMoon
                            onClick={addDark}
                            size={20} className="text-2xl text-gray-500 cursor-pointer" />
                    </div>
                </div>
            </div>
        </header >
